...
const Notifications = ({ symbol }) => {
  ...
  const loadData = async () => {
    if (!symbol) return;
    try {
      const res = await fetch(`http://localhost:5000/api/trades/load/${symbol}`);
      if (res.ok) {
  ...
  const fetchPrice = async () => {
    if (!symbol) return;
    try {
      const res = await fetch(`http://localhost:5000/api/symbol/${symbol}`);
      if (res.ok) {
        const data = await res.json();
        setSymbolData(data);
        if (data['1m']?.length > 0) {
  ...
  const exportReport = () => {
    ...
    a.href = url; a.download = `trade_report_${symbol}_${Date.now()}.json`;
    a.click();
  };
  ...
  const clearData = async () => {
    ...
    try {
      await fetch(`http://localhost:5000/api/trades/clear/${symbol}`, { method: 'DELETE' });
      setTrades([]);
      setOpenPositions([]);
      setTotalPL(0);
    } catch (err) {
  ...
  const getReasonBadge = (reason) => {
    const colors = { SL: 'bg-red-600', PROFIT: 'bg-green-600', REVERSAL: 'bg-yellow-600', MANUAL: 'bg-blue-600' };
    return <span className={`${colors[reason] || 'bg-gray-500'} px-2 py-1 rounded text-xs font-semibold`}>{reason}</span>;
  };

  const calcUnrealizedPL = (pos) => {
    if (!currentPrice) return 0;
    return pos.direction === 'LONG' ? currentPrice - pos.entryPrice : pos.entryPrice - currentPrice;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-green-400 mt-1">🤖 Backend auto-trading active (no browser needed)</p>
          </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          ...
          <div className={`p-4 rounded-lg ${totalUnrealizedPL >= 0 ? 'bg-blue-900/50' : 'bg-orange-900/50'}`}>
            ...
            <div className={`text-2xl font-bold ${totalUnrealizedPL >= 0 ? 'text-blue-400' : 'text-orange-400'}`}>
      ...
      <div className="bg-gray-800 rounded-lg p-4">
        ...
        {openPositions.length === 0 ? (
          <div className="text-center py-6 text-gray-500 border-2 border-dashed border-gray-600 rounded-lg">
            No open positions. Auto trader will enter on valid signals.
          </div>
        ) : (
          <div className="grid gap-3">
            {openPositions.map((pos, idx) => {
              ...
              return (
                <div key={pos.id} className={`p-4 rounded-lg border-2 ${
                  pos.direction === 'LONG' ? 'bg-green-900/20 border-green-600' : 'bg-red-900/20 border-red-600'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <span className="text-xl">{pos.direction === 'LONG' ? '🟢' : '🔴'}</span>
                      <span className={`px-3 py-1 rounded font-bold ${pos.direction === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>
                        {pos.direction}
                      </span>
                      <span className="text-gray-400">#{idx + 1}</span>
                    </div>
                    <div className={`text-xl font-bold ${upl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {upl >= 0 ? '+' : ''}{upl.toFixed(2)} pts
                    </div>
                  </div>
                  <div className="grid grid-cols-5 gap-4 mt-3 text-sm">
                    <div>
                      <span className="text-gray-400">Entry:</span>
                      <span className="ml-2 font-mono">{pos.entryPrice.toFixed(2)}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">Current:</span>
                      <span className="ml-2 font-mono text-yellow-400">{currentPrice.toFixed(2)}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">SL:</span>
                      <span className="ml-2 font-mono text-red-400">{pos.stopLoss.toFixed(2)}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">Signal:</span>
                      <span className="ml-2">{pos.strength}</span>
                    </div>
                    <div>
                      <span className="text-gray-400">Time:</span>
                      <span className="ml-2">{formatTime(pos.entryTime)}</span>
                    </div>
                  </div>
                </div>
              );
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden">
        <div className="p-4 border-b border-gray-700">
          <h3 className="text-lg font-bold">📋 Trade History ({trades.length} trades)</h3>
        </div>
        <table className="w-full">
          <thead className="bg-gray-700">
            <tr>
              ...
              <th className="px-4 py-3 text-right">P/L (pts)</th>
            ...
            {trades.length === 0 ? (
              ...
              <tr key={t.id} className={`border-t border-gray-700 ${i % 2 === 0 ? 'bg-gray-800' : 'bg-gray-750'}`}>
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${t.direction === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>
                    {t.direction}
                  </span>
                </td>
                ...
                <td className={`px-4 py-3 text-right font-bold ${t.pl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
